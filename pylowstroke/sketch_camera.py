...
def findFirstVisibleStrokeTowardsVP(probabilitiesVP, sketch):
    ...
    while sketch.strokes[ind_stroke].line_group != 0 or lines_group[lj] == 3 or\
          sketch.strokes[ind_stroke].mean_pressure < 0.05:
...
def estimate_camera_parameters(vps, sketch_height, point_2d_counterpart_origin):
    ...
    #print("principal_point_vp", principal_point_vp)
    ...
    p1 = P_vp@np.array([1, 0, 0, 1])
    p1 = p1/p1[2]
    p2 = P_vp@np.array([0, 1, 0, 1])
    p2 = p2/p2[2]
    p3 = P_vp@np.array([0, 0, 1, 1])
    p3 = p3/p3[2]
    #print(p1)
    #print(p2)
    #print(p3)
    ...
    return cam_param, vp, np.array(xyz_inds, dtype=np.int)
